...
public class LakehousePlugin extends Plugin implements SchemaContributor, ExternalTableExecutor, ActionPlugin, Closeable {
    ...
    public ExternalScanContext prepareScan(RelNode logicalPlan, ExternalTable externalTable) {
        ...
        // Only pushes filters directly above a TableScan (WHERE clauses).
        // HAVING filters above aggregates reference aliases that don't exist in the table schema.
        ...
        // Credentials are set on ThreadLocal so LakehouseCredentialsProvider
        // returns them to the Iceberg SDK. The privileged executor propagates
        ...
        logger.info("[LakehousePlugin] Scan plan: {} files, {} bytes total", scanPlan.fileCount(), scanPlan.getTotalFileSize());
        ...
        try {
            ...
            // but DataFusion registers tables with just the leaf name.
            sqlQuery = stripSchemaQualifiers(sqlQuery, tableName);
            logger.debug("[LakehousePlugin] Generated SQL for DataFusion: {}", sqlQuery);
        } catch (Exception e) {
