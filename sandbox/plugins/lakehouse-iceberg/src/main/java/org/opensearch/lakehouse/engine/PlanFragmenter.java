...
public final class PlanFragmenter {
    ...
    private static SubPlan fragmentAggregate(PlanVisitor visitor, String sql, RelNode relNode) {
        ...
        if (hasGroupBy && hasNonPassthroughAnyValue(aggregate, isPassthrough)) {
            throw new UnsupportedOperationException(
                "GROUP BY with non-passthrough ANY_VALUE (constant expressions) is not distributable — "
                    + "coordinator column mapping assumes group keys are first but original SQL may differ"
            );
        }
