...
final class AbsorbRuleUtils {
    ...
    private static final Logger logger = LogManager.getLogger(AbsorbRuleUtils.class);
    ...
    static RelNode replaceWithFragment(RelNode node) {
        ...
        RelNode result = node.copy(node.getTraitSet(), newInputs);
        logger.info("[AbsorbRuleUtils] replaceWithFragment: node={}, inputCount={}, newInputCount={}, resultInputCount={}",
            node.getClass().getSimpleName(), node.getInputs().size(), newInputs.size(), result.getInputs().size());
        return result;
    }
